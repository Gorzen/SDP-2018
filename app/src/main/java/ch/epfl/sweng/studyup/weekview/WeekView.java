package ch.epfl.sweng.studyup.weekview;
...
import static ch.epfl.sweng.studyup.weekview.WeekViewUtil.isSameDay;
import static ch.epfl.sweng.studyup.weekview.WeekViewUtil.isSameDayAndHour;
import static ch.epfl.sweng.studyup.weekview.WeekViewUtil.today;
...
//                    mScroller.fling((int) mCurrentOrigin.x, 0, (int) (velocityX * mXScrollingSpeed), 0, (int) getXMinLimit(), (int) getXMaxLimit(), 0, 0);
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, (int) (velocityX * mXScrollingSpeed), 0, (int) getXMinLimit(), (int) getXMaxLimit(), (int) mCurrentOrigin.y, (int) mCurrentOrigin.y);
...
//                    mScroller.fling(0, (int) mCurrentOrigin.y, 0, (int) velocityY, 0, 0, (int) getYMinLimit(), (int) getYMaxLimit());
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, 0, (int) velocityY, (int) mCurrentOrigin.x, (int) mCurrentOrigin.x, (int) getYMinLimit(), (int) getYMaxLimit());
                    break;
            }
...
//                        selectedTime.add(Calendar.MINUTE, mod < Math.ceil(mNewEventTimeResolutionInMinutes / 2) ? -mod :
//                                (mNewEventTimeResolutionInMinutes - mod));
//
//                        Calendar endTime = (Calendar) selectedTime.clone();
//                        endTime.add(Calendar.MINUTE, Math.min(mNewEventLengthInMinutes, (24-selectedTime.get(Calendar.HOUR_OF_DAY))*60 -
//                                selectedTime.get(Calendar.MINUTE)));
//                        WeekViewEvent newEvent = new WeekViewEvent(mNewEventId, mNewEventText, null, selectedTime, endTime);

                        int unroundedMinutes = selectedTime.get(Calendar.MINUTE);
                        // int mod = unroundedMinutes % mNewEventTimeResolutionInMinutes;
                        selectedTime.add(Calendar.MINUTE, -unroundedMinutes);
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            String time = getDateTimeInterpreter().interpretTime(i + 8);
            if (time == null)
                throw new IllegalStateException("A DateTimeInterpreter must not return null time");
    ...
    private void drawHeaderRowAndEvents(Canvas canvas)
    {
        ...
        if (mAreDimensionsInvalid) {
            mEffectiveMinHourHeight= Math.max(mMinHourHeight, (int) ((getHeight() - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom) / 12));
...
//TODO            if(mNumberOfVisibleDays >= 7 && today.get(Calendar.DAY_OF_WEEK) != mFirstDayOfWeek && mShowFirstDayOfWeekFirst) {
//                int difference = (today.get(Calendar.DAY_OF_WEEK) - mFirstDayOfWeek);
        ...
        if (mCurrentOrigin.y < getHeight() - mHourHeight * 12 - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight/2)
            mCurrentOrigin.y = getHeight() - mHourHeight * 12 - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight/2;
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) && !mEventRects.get(i).event.isAllDay()){
                    ...
                    // Calculate top.
                    float top = mHourHeight * 12 * mEventRects.get(i).top / 1440 + mCurrentOrigin.y + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 + mEventMarginVertical;

                    // Calculate bottom.
                    float bottom = mEventRects.get(i).bottom;
                    bottom = mHourHeight * 12 * bottom / 1440 + mCurrentOrigin.y + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - mEventMarginVertical;
